    ...
    <div class="__cart-total __cart-total_sticky">
        <div class="cart_total p-0">
            ...
            <div class="d-flex justify-content-between">
                ...
                <span class="cart_value" id="cart-subtotal">
            ...
            <div class="d-flex justify-content-between">
                ...
                <span class="cart_value" id="cart-tax">
            ...
            <div class="d-flex justify-content-between">
                ...
                <span class="cart_value" id="cart-shipping-cost">
                    @php($sessionShippingCost = session('shipping_cost', 0))
                    @if($sessionShippingCost > 0)
                        {{ number_format($sessionShippingCost, 2) }}
                    @else
                        0.00
                    @endif
                </span>
            ...
            <!-- Service Fee -->
            @php($serviceFeeSetting = \App\Models\BusinessSetting::where('type', 'service_fee')->first())
                @php($serviceFeePercent = (float)($serviceFeeSetting->value ?? 0))
                @php($serviceFeeAmount = $sessionShippingCost * $serviceFeePercent / 100)

                <div class="d-flex justify-content-between">
                    <span class="cart_title">{{ translate('Service Fee') }}</span>
                    <span class="cart_value" id="cart-service-fee">
                    {{ number_format($serviceFeeAmount, 2) }}
                </span>
                </div>
                ...
                <div class="d-flex justify-content-between">
                    ...
                    <span class="cart_value" id="cart-discount">
                ...
                @if(auth('customer')->check())
                    @if(session()->has('coupon_discount'))
                        @php($couponDiscount = session('coupon_discount'))
                ...
                <hr class="my-2">
                <!-- Total -->
                @php($calculatedTotal = $subTotal + $totalTax + $sessionShippingCost + $serviceFeeAmount - $coupon_dis - $totalDiscountOnProduct - $orderWiseShippingDiscount)
                <div class="d-flex justify-content-between">
                    <span class="cart_title text-primary font-weight-bold">{{ translate('total') }}</span>
                    <span class="cart_value text-primary font-weight-bold" id="cart-total-amount">
                    {{ webCurrencyConverter(amount: $calculatedTotal) }}
                </span>
                </div>
        </div>
...
<div class="bottom-sticky3 bg-white p-3 shadow-sm w-100 d-lg-none">
    @php($calculatedTotalMobile = $subTotal + $totalTax + $sessionShippingCost + $serviceFeeAmount - $coupon_dis - $totalDiscountOnProduct - $orderWiseShippingDiscount)
    <div class="d-flex justify-content-center align-items-center fs-14 mb-2">
        ...
        <strong class="text-base">
            {{ webCurrencyConverter(amount: $calculatedTotalMobile) }}
        </strong>
    </div>
