...
@php( $user = \App\Models\User::find(auth('customer')->id()))
...
<div class="navbar-tool dropdown me-2 {{Session::get('direction') === "rtl" ? 'mr-md-3' : 'ml-md-3'}}">
    @if($web_config['guest_checkout_status'] || auth('customer')->check())
        <a class="navbar-tool-icon-box bg-secondary dropdown-toggle" href="{{route('shop-cart') }}">
            <span class="navbar-tool-label" style="background-color: #140b49 !important;">
                {{ $cart->count() }}
            </span>
                                ...
                                <div
                                    ...
                                    @if( isset($product->status) && $product->status == 1)
                                        <div class="__quantity">
                                            ...
                                            <div class="quantity__minus cart-qty-btn action-update-cart-quantity disabled"
                                            ...
                                            <div class="quantity__plus cart-qty-btn action-update-cart-quantity disabled"
                                                data-cart-id="{{ $cartItem['id'] }}"
                                                data-product-id="{{ $cartItem['product_id'] }}"
                                                data-action="1"
                                                data-event=""
