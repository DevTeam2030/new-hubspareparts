...
@section('content')
    <div class="content container-fluid">
        ...
        @include('admin-views.blog.partials._blog-tab-menu')
        ...
        @include('admin-views.blog.partials._blog-intro-section')

        <div class="card mb-3">
            <div class="card-body">
                @include('admin-views.blog.partials._blog-filter-section')

                <div class="d-flex align-items-stretch flex-wrap justify-content-between gap-3 mb-20">
                    <h4 class="m-0 flex-grow-1 d-flex gap-2 align-items-center">{{ translate('Blog_List') }}
                        @if(count($blogs) > 0)
                ...
                @if(count($blogs) > 0)
                    @include('admin-views.blog.partials._blog-list-section')
                @else
...
@push('script')
    <script>
        // Handle custom-modal-plugin for input-change-form
        $('.custom-modal-plugin[data-modal-type="input-change-form"]').on('change', function (event) {
            event.preventDefault();
            const $this = $(this);
            const formId = $this.data('modal-form');
            const modalId = 'toggle-status-modal';
            ...
            const onButtonText = $this.data('on-button-text');
            const offButtonText = $this.data('off-button-text');
            
            // Set modal content
            if ($this.is(':checked')) {
                $('#' + modalId + '-title').text(onTitle);
                $('#' + modalId + '-message').html(onMessage);
                $('#' + modalId + '-image').attr('src', onImage);
                $('#' + modalId + '-ok-button').text(onButtonText);
            } else {
                $('#' + modalId + '-title').text(offTitle);
                $('#' + modalId + '-message').html(offMessage);
                $('#' + modalId + '-image').attr('src', offImage);
                $('#' + modalId + '-ok-button').text(offButtonText);
            }
            
            // Store form reference
            $('#' + modalId + '-ok-button').attr('data-form', formId.replace('#', ''));
        ...
        $('.blog-status-form:not(#blog-custom-status-form), .single-blog-status-form').on('submit', function (event) {
            event.preventDefault();
            $.ajax({
