...
class HomeController extends Controller
{
    ...
    public function default_theme(): View
    {
        ...
        // Featured Products Query with Governorate Filtering
        $featuredProductsList = ProductManager::getPriorityWiseFeaturedProductsQuery(
            query: $this->product->active()
                ->with(['clearanceSale' => function ($query) {
                    // Only load active clearanceSale data
                    return $query->active();
                }]),
                // Governorate filtering: only get products from sellers whose shop's governorate_id matches the logged-in user's governorate_id
...
//                        $query
//
//                            ->whereHas('seller', fn($q) =>
//                            $q->whereHas('governorates', fn($q2) =>
//                            $q2->whereIn('id', $govs)
//                            )
//                            )
...
// New Arrival Products Query with Governorate Filtering
        $newArrivalProducts = ProductManager::getPriorityWiseNewArrivalProductsQuery(
            query: $this->product->active()
                ->with(['clearanceSale' => function ($query) {
                    // Only load active clearanceSale data
                    return $query->active();
...
//                }),
        ...
        $dealOfTheDay = DealOfTheDay::with(['product' => function($query) {
            // Load only active product data along with its active clearance sale details
...
//                if (! empty($govs)) {
//                    $query
//                        ->whereHas('product.seller.governorates', fn($q) => $q->whereIn('id', $govs))
//                        ->whereHas('product.seller.shop.governorates', fn($q) => $q->whereIn('id', $govs));
//                }
//            })
