...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request): JsonResponse|RedirectResponse
    {
        ...
        $previousRouteName = $previousRoute->getName(); // route name (if exists)
        $user = \App\Models\User::find(auth('customer')->id());
        if($previousRouteName!='view-wishlist-collection' && !$user->hasPermissionTo('web-add-to-cart') ){
            if ($request->ajax()) {
                return response()->json([
                    'status' => 0,
                    'message' => 'You do not have permission to add to cart'
                ]);
            }

            Toastr::error("You do not have permission to add to cart");
            return back();
        }
    ...
    public function removeFromCart(Request $request): JsonResponse
    {
        ...
        if (!$user->hasPermissionTo('web-export-to-cart')) {
            return response()->json([
                ...
                'message' => translate('you do not have permission to remove from cart'),
            ]);
        }

        $user = Helpers::getCustomerInformation();
        $customerId = ($user == 'offline' ? session('guest_id') :  auth('customer')->id());

        // Get the cart item that will be removed
        $cartItem = Cart::where([
            'id'          => $request->key,
        ...
        if ($cartItem) {
            // If logged in, also remove from related accounts
            if ($user != 'offline') {
                $relatedAccounts = self::getRelatedAccounts(); // adjust relation
    ...
    public function updateQuantity(Request $request)
    {
        ...
        if (!$user->hasPermissionTo('web-export-to-cart')) {
            return response()->json([
                'type' => 'error',
                'msg' => __("You do not have permission to update cart"),
            ]);
        }
    ...
    public function remove_all_cart()
    {
        $user = \App\Models\User::find(auth('customer')->id());
        if (!$user->hasPermissionTo('web-export-to-cart')) {
            Toastr::error(translate('you do not have permission to remove from cart'));
