...
class WebController extends Controller
{
    ...
    public function getAllVendorsView(Request $request): View|RedirectResponse
    {
        ...
        if (auth('customer')->check()) {
            $govs = [ auth('customer')->user()->governorate_id ];
        } elseif (auth('seller')->check()) {
            $govs = auth('seller')->user()->governorates()->pluck('id')->toArray();
        } else {
            $govs = [];
        }
            $vendorsList = Shop::active()
                ->withCount('products')
                ->with(['seller' => fn($q) => $q->withCount('orders')])
                ->when(! empty($govs), function ($q) use ($govs) {
                    $q->whereHas('seller.governorates', fn($q2) => $q2->whereIn('id', $govs))
                        ->whereHas('seller.shop.governorates', fn($q2) => $q2->whereIn('id', $govs));
                })
...
//        ])->get();
        $shippingAddresses = ShippingAddress::whereIn('customer_id', $relatedAccounts)
            ->where('is_guest', $user === 'offline')
            ->get();
        ...
        $governorates = Governorate::find($user->governorate_id);
...
//        }
    ...
    public function getCashOnDeliveryCheckoutComplete(Request $request): View|RedirectResponse|JsonResponse
    {
        ...
        if ($physicalProductExist) {
            ...
            session()->forget('selected_delivery_time');
    ...
    public function getQuickView(Request $request): JsonResponse
    {
        ...
        $wishlists = Wishlist::where('wishlist_collection_id' , '!=' , null)->where('product_id', $product->id)->get();
        ...
        return response()->json([
            ...
            'view' => view(VIEW_FILE_NAMES['product_quick_view_partials'], compact('product', 'countWishlist', 'countOrder',
                ...
                'inhouse_vacation_start_date', 'inhouse_vacation_end_date', 'inHouseVacationStatus', 'inhouse_temporary_close', 'wishlist_status', 'overallRating', 'rating', 'firstVariationQuantity' , 'relatedAccounts'))->render(),
        ]);
    }
    ...
    public function storeWishlist(Request $request)
    {
        ...
        if (!$user->hasPermissionTo('web-adddelete-products-to-wishlist')) {
            ...
            return response()->json([
                'error' => translate("You do not have permission to add to wishlist"),
                'value' => 3,
                'count' => $countWishlist,
        ...
        if ($request->collection_id == null) {
            ...
            $product_count = Wishlist::where(['product_id' => $request->product_id])->count();
            return response()->json([
                'error' => translate("You should select collection"),
                'value' => 5,
                'count' => $countWishlist,
                'product_count' => $product_count
            ]);
        ...
        if($request->collection_id != null){
            $collection = \App\Models\WishlistCollection::find($request->collection_id);
            $countWishlist = Wishlist::whereHas('wishlistProduct', function ($q) {
                return $q;
            })->where('customer_id', auth('customer')->id())->count();
            $product_count = Wishlist::where(['product_id' => $request->product_id])->count();
            if($collection->eng_approve !=null || $collection->eng_proc!=null){
                return response()->json([
                    'error' => translate("can not add to approve collection"),
                    'value' => 4,
                    'count' => $countWishlist,
                    'product_count' => $product_count
                ]);
        ...
        if ($request->ajax()) {
            if (auth('customer')->check()) {
                ...
                if ($wishlist) {
                    ...
                    return response()->json([
                        'error' => translate("it is added before"),
                        'value' => 2,
                        'count' => $countWishlist,
                        'product_count' => $product_count
                    ]);
    ...
    public function deleteWishlist(Request $request): JsonResponse
    {
        $user = User::find(auth('customer')->id());
        if (!$user->hasPermissionTo('web-adddelete-products-to-wishlist')) {
            return response()->json(['error' => translate('You do not have permission to delete from wishlist'), 'value' => 0]);
        }
    ...
    public function deleteAllWishListItems(): RedirectResponse
    {
        $user = User::find(auth('customer')->id());
        if (!$user->hasPermissionTo('web-adddelete-products-to-wishlist')) {
            Toastr::error(translate('You do not have permission to delete from wishlist'));
    ...
    public function checkValidationForCheckoutPages(Request $request): array
    {
        ...
        foreach ($cartItemGroupIDs as $groupId) {
            ...
            if ($isPhysicalProductExist) {
                foreach ($cartList as $cart) {
                    // Determine shippingType as before
                    ...
                    // 🔧 Skip the error-check block entirely when it's order_wise
                    if ($shippingType === 'order_wise') {
                        continue;
                    }

                    // Original validation for non–order_wise shipping types
                    ...
                    if ($shippingMethod == 'inhouse_shipping') {
                        ...
                        if ($sellerShippingCount <= 0 && isset($cart->seller->shop)) {
                            $message[]            = translate('shipping_Not_Available_for')
                        ...
                        if ($cart->seller_is == 'admin') {
                            ...
                            $sellerShippingCount = ShippingMethod::where('status', 1)
                                ...
                                ->where('creator_id',   $cart->seller_id)
                                ->count();
    ...
    public function updateWishlistQuantity(Request $request)
    {
        if (!auth('customer')->check()) {
            return response()->json([
                'error' => translate('please_login_your_account'),
                'value' => 0
            ], 401);
        }
        ...
        if (!$user->hasPermissionTo('web-adddelete-products-to-wishlist')) {
            return response()->json([
                'error' => translate("You do not have permission to update wishlist"),
                'value' => 3 // Permission denied code
            ], 403);
        }
        ...
        $wishlist = Wishlist::where('id', $request->wishlist_id)
            ->whereIn('customer_id', $relatedAccounts)
            ->first();

        if (!$wishlist) {
            return response()->json([
                'error' => translate('Wishlist item not found'),
                'value' => 0
            ], 404);
        }

        // Check if collection is approved
        if ($wishlist->wishlist_collection_id) {
            $collection = \App\Models\WishlistCollection::find($wishlist->wishlist_collection_id);
            if ($collection && ($collection->eng_approve != null || $collection->eng_proc != null)) {
                return response()->json([
                    'error' => translate("Cannot update approved collection"),
                    'value' => 4 // Collection approved code
                ], 403);
            }
        }
        ...
        if (!$product) {
            return response()->json([
                'error' => translate('Product not found'),
                'value' => 0
            ], 404);
        }
        ...
        if ($product->current_stock < $request->quantity) {
            return response()->json([
                'error' => translate('Requested quantity exceeds available stock'),
                'value' => 0,
        ...
        return response()->json([
            'success' => translate('Quantity updated successfully'),
            'value' => 1, // Success code
    ...
    public function getQuantity(Request $request)
    {
        ...
        if ($wishlist) {
            return response()->json(['quantity' => $wishlist->quantity]);
        }
