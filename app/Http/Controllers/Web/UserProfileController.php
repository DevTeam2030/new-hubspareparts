...
class UserProfileController extends Controller
{
    ...
    public function user_account(Request $request)
    {
        ...
        if (!$user->hasPermissionTo('web-edite-profile')) {
            return redirect()->back()->with(['msg' => __('You do not have permission to access this page'), 'type' => 'error']);
        }
        ...
        return view(VIEW_FILE_NAMES['user_account'], compact('customerDetail','governorates'));
    ...
    public function getUserProfileUpdate(CustomerProfileUpdateRequest $request): RedirectResponse
    {
        ...
        $oldGov   = $user->governorate_id;

        $imageName = $request->file('image')
            ? ImageManager::update('profile/', $user->image, 'webp', $request->file('image'))
            : $user->image;
        ...
        if ($oldGov !== $request->governorate_id) {
    ...
    {
//        $user =  User::where('id', auth('customer')->id())->first();
//        if (!$user->hasPermissionTo('web-shipping-address')) {
//            Toastr::warning(translate('You do not have permission to access this page'));
//            return back();
//        }
        $country_restrict_status = getWebConfig(name: 'delivery_country_restriction');
        $zip_restrict_status = getWebConfig(name: 'delivery_zip_code_area_restriction');
        ...
        if (auth('customer')->check()) {
            $shippingAddresses = ShippingAddress::whereIn('customer_id', $this->getRelatedAccounts())
                ->latest()->get();
            return view('web-views.users-profile.account-address', compact('shippingAddresses', 'country_restrict_status', 'zip_restrict_status', 'countries', 'zip_codes', 'countriesName', 'countriesCode' , 'governorates'));
        } else {
            return redirect()->route('home');
        }
    }
    ...
    public function address_edit(Request $request, $id)
    {
        ...
        if (isset($shippingAddress)) {
            return view(VIEW_FILE_NAMES['account_address_edit'], compact('shippingAddress', 'country_restrict_status', 'zip_restrict_status', 'delivery_countries', 'delivery_zipcodes', 'countriesName', 'countriesCode' ,'governorates'));
        } else {
            Toastr::warning(translate('access_denied'));
            return back();
        }
        ...
        if (theme_root_path() == 'theme_fashion') {
            ...
            $orders = $this->order->withSum('orderDetails', 'qty')
                //->where(['customer_id' => auth('customer')->id(), 'is_guest' => '0'])
                      ->whereIn('customer_id', $this->getRelatedAccounts())
    ...
    public function account_order_details(Request $request , $id): View|RedirectResponse
    {
//        $order = $this->order->with(['deliveryManReview', 'customer', 'offlinePayments', 'details.productAllStatus'])
//            ->where(['id' => $id, 'customer_id' => auth('customer')->id(), 'is_guest' => '0'])
//            ->first();
        $order = $this->order->with(['deliveryManReview', 'customer', 'offlinePayments', 'details.productAllStatus'])
            ->where(['id' => $id, 'is_guest' => '0'])
            ->whereIn('customer_id', $this->getRelatedAccounts())
            ->first();
    ...
    public function account_tickets()
    {
        if (auth('customer')->check()) {
            $user =  User::where('id', auth('customer')->id())->first();
//            if (!$user->hasPermissionTo('web-support-ticket')) {
//                return redirect()->back()->with(['msg' => __('You do not have permission to access this page'), 'type' => 'error']);
//            }
    ...
    public function submitSupportTicket(Request $request): RedirectResponse
    {
        ...
        if (!$user->hasPermissionTo('web-support-ticket')) {
            return redirect()->back()->with('error',  __('You do not have permission to access this page'));
        }
        $request->validate([
    ...
    public function generate_invoice($groupId)
    {
        // 1) Fetch data exactly مثل generate_invoice
        $orders = Order::with([
        ...
        // 2) إجمالي الشحن والخدمة والمنتجات
        $totalShippingCost   = $order->shipping_cost;
        $totalServiceFee     = $order->service_fee;
        $totalOrdersAmount = $orders->sum('order_amount');

        $grandTotal = $totalOrdersAmount
            + $totalShippingCost
            + $totalServiceFee;

        // 3) إعدادات الفاتورة واللوجو
        $invoiceSettings = json_decode(
            $this->businessSettingRepo
                ->getFirstWhere(['type' => 'invoice_settings'])?->value,
            true
        );
        $logoData = ''; // لو كنت تستخدم base64
        // أو
        $logoUrl  = asset('images/logo.png');

        // 4) رجّع الـview
        return view('web-views.order.invoice', [
            'orders'              => $orders,
            'order'               => $order,
            ...
            // تمرير اللوجو بنفس المفتاح اللي تستخدمه في البلید
            'logoData'            => $logoData,
            'logo'                => $logoUrl,
        ]);
    }
    ...
    public function account_roles(Request $request)
    {
        $roles = Role::where('guard_name' , 'web')->with('permissions')->get();
        $roleAllowedPermissions = Permission::where('guard_name' , 'web' )->get();
        return view(VIEW_FILE_NAMES['account_roles'], compact('roles','roleAllowedPermissions'));
    }
    ...
    public function updateRoles(Request $request, $id)
    {
        ...
        // Convert IDs to permission names if needed
        $permissions = $request->permissions ?? [];

        // If permissions are numeric, fetch their names
        ...
        Toastr::success(translate('Role updated.'));
        return back();
    }
    ...
    public function allUsers()
    {
        ...
        where(function ($query) {
            $query->whereIn('parent_id', $this->getRelatedAccounts())
                ->orWhereIn('id', $this->getRelatedAccounts());
        })

            ->paginate(10);
        $user = User::with('roles.permissions')->find(auth('customer')->id());
        $governorates = Governorate::get();
        $roles = Role::where('guard_name' , 'web')->get();
        return view(VIEW_FILE_NAMES['account_users'], compact('all_users','user' , 'governorates' , 'roles'));
    }
    ...
    public function storeUser(Request $request): \Illuminate\Http\RedirectResponse
    {
        ...
        $request->validate([
            ...
            [
                'f_name.required' => __('name is required'),
                'f_name.max' => __('name is must be between 191 character'),
                'l_name.required' => __('name is required'),
                'l_name.max' => __('name is must be between 191 character'),
                'email.unique' => __('email is already taken'),
                'email.required' => __('email is required'),
                'password.required' => __('password is required'),
                'password.confirmed' => __('both password does not matched'),
                'password.min' => __('password  is at least 8 chars'),
        ...
        Toastr::success(translate('New User Added.'));
    ...
    public function editUser($id)
    {
        ...
        return view(VIEW_FILE_NAMES['account_edit_user'], compact('governorates' , 'roles' , 'user'));
    }

    public function updateUser(Request $request): \Illuminate\Http\RedirectResponse
    {

        $authUser = User::find(auth('customer')->id());
        ...
        $request->validate([
            ...
            'password' => ['nullable', 'string', 'min:8', 'confirmed'], // Changed to nullable for edit
            'phone' => [
                'required',
                'string',
                Rule::unique('users')->ignore($user->id)
...
//            ->firstOrFail();
//        $user->syncRoles($role);
        $newRole = Role::where('id', $request['role'])
            ->where('guard_name', 'web')
            ->firstOrFail();
        ...
        //return redirect()->route('management.user.all')->with('success', 'User created.');
        Toastr::success(translate(' User Updated .'));
        return redirect()->route('account-users');
    }
    ...
    public function deleteUser($id)
    {
        ...
        ShippingAddress::where('customer_id' , $id)->delete();
        $user = User::find($id);
        $user->delete();
        Toastr::success(translate(' User deleted .'));
        return redirect()->route('account-users');
    }
    public function wishlistCollections()
    {

        $all_collections = WishlistCollection::with('user')
           ->with('user_eng_approve')->with('user_eng_proc')
          ->orderBy('id')
          ->whereIn('user_id', $this->getRelatedAccounts())->get();

        return view(VIEW_FILE_NAMES['account_wishlist_collections'] , compact('all_collections') );
    }


    public function createWishlistCollection()
    {

        return view(VIEW_FILE_NAMES['wishlist_collections_add']  );
    }

    public function storeWishlistCollection(Request $request): \Illuminate\Http\RedirectResponse
    {
        ...
        $request->validate([
            ...
            'arabic_name' => ['nullable', 'string', 'max:191'],
            ...
            [
                ...
                'arabic_name.max' => __('name ar is must be between 191 character'),
                'due_date.required' => __('due date is required'),
                'priority.required' => __('priority is required'),
            ]
        );
        $collection = \App\Models\WishlistCollection::create([
            'name' => $request['name'],
            'name_ar' => $request['arabic_name'],
        ...
        Toastr::success(translate('New collection Added.'));
        return redirect()->route('account-wishlist-collections');
    }

    public function editWishlistCollection($id)
    {
        $collection = WishlistCollection::find($id);
        $user = User::find($collection->user_id);

        return view(VIEW_FILE_NAMES['wishlist_collections_edit'], compact('collection'  , 'user'));
    }

    public function updateWishlistCollection( Request $request): \Illuminate\Http\RedirectResponse
    {
        ...
        $collection = WishlistCollection::find($request->id);
        $request->validate([
            'name' => ['required', 'string', 'max:191'],
            'arabic_name' => ['nullable', 'string', 'max:191'],
            'due_date' => ['required', 'date'],
        ...
        $collection->name_ar =  $request['arabic_name'];
        $collection->due_date =  $request['due_date'];
        $collection->priority =  $request['priority'];
        $collection->notes =  $request['notes'];
        $collection->eng_approve =  ($request['eng_approve']!='') ? Carbon::now()->toDateString() : null;
        if($request['eng_approve_user_id']!=''){
            $collection->eng_approve_user_id =  $request['eng_approve_user_id'];
        }else{
            $collection->eng_approve_user_id =  ($request['eng_approve']!='') ?auth('customer')->id() : null;
        }
        $collection->eng_proc =  ($request['eng_proc']!='') ? Carbon::now()->toDateString() : null;
        if($request['eng_proc_user_id']!=''){
            $collection->eng_proc_user_id =  $request['eng_proc_user_id'];
        }else{
            $collection->eng_proc_user_id =  ($request['eng_approve']!='') ? auth('customer')->id() : null;
        }
        $collection->save();
        Toastr::success(translate('collection Updated.'));
        //return redirect()->route('account-wishlist-collections');
        return redirect()->route('view-wishlist-collection', ['id' => $collection->id]);
    }

    public function deleteWishlistCollection($id)
    {

        $user = User::find(auth('customer')->id());
        if (!$user->hasPermissionTo('web-viewcreat-edite-remove-collection')) {
            Toastr::error(translate('You do not have permission to access this page.'));
            return redirect()->route('account-wishlist-collections');

        }
        $collection = WishlistCollection::find($id);
        Wishlist::where('wishlist_collection_id' , $collection->id)->delete();
        $collection->delete();
        Toastr::success(translate('collection deleted.'));
        return redirect()->route('account-wishlist-collections');
    }

    public function viewWishlistCollection($id)
    {

        $user = User::find(auth('customer')->id());
        if (!$user->hasPermissionTo('web-view-collection')) {
            return redirect()->back()->with('error',  __('You do not have permission to access this page'));
        }
        $collectionId = $id;
    ...
    public function approveEng(Request $request)
    {

        $collection = \App\Models\WishlistCollection::find($request->wishlist_id);
        $collection->eng_approve = ($request->approved==1) ? Carbon::now()->toDateString() : null ;
        $collection->eng_approve_user_id = ($request->approved==1) ? \auth('customer')->id() : null;
        $collection->save();
        return response()->json(['type' => 'success', 'msg' => 'collection updated']);
    }

    public function approveProc(Request $request)
    {
        $request->validate([
        ...
        $collection = \App\Models\WishlistCollection::find($request->wishlist_id);
        $collection->eng_proc = ($request->approved==1) ? Carbon::now()->toDateString() : null;
    ...
    public function getRelatedAccounts()
    {
        ...
        if($user->parent_id != null){
            $usersArray[] = User::where('parent_id' , $user->parent_id)->pluck('id')->toArray();
        }else{
            $usersArray[] = User::where('parent_id' , $user->id)->pluck('id')->toArray();

        }
        $usersArray[] = $user->id;
        if($user->parent_id!=null){
            $usersArray[] = User::where('id' , $user->parent_id)->pluck('id')->toArray();
        }
        return Arr::flatten($usersArray);
    }
