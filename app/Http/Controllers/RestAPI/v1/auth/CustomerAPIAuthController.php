...
class CustomerAPIAuthController extends Controller
{
    ...
    public function passwordResetRequest(Request $request): JsonResponse
    {
        ...
        // 1) نستخرج العميل بناءً على النوع (phone أو email)
        if ($request['type'] == 'phone') {
            $customer = $this->customerRepo->getFirstWhere(['phone' => $request['email_or_phone']]);
        } else {
            $customer = $this->customerRepo->getFirstWhere(['email' => $request['email_or_phone']]);
        }
        ...
        // 2) نتحقق من توقيت إعادة إرسال الـOTP (للهاتف)
        $OTPIntervalTime = getWebConfig(name: 'otp_resend_time') ?? 60; // بالثواني
        $passwordVerificationData = DB::table('password_resets')
            ->where('identity', $request['email_or_phone'])
            ->first();

        if (isset($passwordVerificationData)
            && Carbon::parse($passwordVerificationData->created_at)->diffInSeconds() < $OTPIntervalTime) {
        ...
        // 3) نولّد التوكن. لو email نستخدم سلسلة طويلة، لو phone نستخدم رقم 6 خانات
        ...
        // 4) نخزّن التوكن في جدول password_resets
        ...
        // 5) نخزّن نفس التوكن في جدول phone_or_email_verifications (لو استعماله مطلوب)
        ...
        // 6) لو النوع email نبعت إيميل
        if ($request['type'] == 'email') {
            // (أ) نولّد رابط إعادة تعيين كلمة السر
            $resetUrl = route('customer.auth.reset-password', [
                'identity' => base64_encode($customer['email']),
                'token' => $token,
            ]);

            // (ب) نجيب إعدادات القالب من GetWebConfig
        ...
        // (ما المفروضش نوصل هنا لأن إما phone أو email)
        return response()->json(['errors' => [
            ['code' => 'unknown', 'message' => translate('Unknown_reset_type')]
        ]], 400);
    }
