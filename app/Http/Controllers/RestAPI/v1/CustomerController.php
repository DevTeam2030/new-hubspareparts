...
class CustomerController extends Controller
{
    ...
    public function update_profile(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'governorate_id.exists'   => translate('Selected governorate is invalid!'),
        ]);
        if ($validator->fails()) {
            return response()->json(['errors' => Helpers::validationErrorProcessor($validator)], 403);
        }
        ...
        $userDetails = [
            ...
            'l_name' => $request->l_name ?? $user->f_name,
            'image' => $imageName,
            ...
            'password' => $pass,
            'governorate_id'      => $request->governorate_id ?? $user->governorate_id ,
            'updated_at' => now(),
        ];
