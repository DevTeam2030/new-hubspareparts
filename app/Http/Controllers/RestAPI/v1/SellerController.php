    ...
    public function getSellerList(Request $request, $type)
    {
        $sellers = $this->seller
            ...
            ->each(function ($seller) {
                ...
                $seller['average_rating'] = $seller['total_rating'] / max($seller['rating_count'], 1);
                ...
                $seller['is_vacation_mode_now'] = $vacationStatus && ($currentDate >= $vacationStartDate) && ($currentDate <= $vacationEndDate)
                    ? 1
                    : 0;

                // إظهار أسماء محافظات التاجر والمحلات المرتبطة به
                $seller['seller_governorates'] = $seller->governorates->map(fn($g) => ['id' => $g->id, 'name' => $g->name])->toArray();
                $seller['shop_governorates']   = $seller->shop->governorates->map(fn($g) => ['id' => $g->id, 'name' => $g->name])->toArray();
        ...
        $inhouseProducts = Product::active()
            ->with(['reviews', 'rating'])
        ...
        $inhouseRattingPositive  = 0;
        foreach ($inhouseReviewData->pluck('rating') as $r) {
            if ($r >= 4) {
                $inhouseRattingPositive++;
            }
        }
        ...
        $inhouseSeller->positive_review = $inhouseReviewDataCount
            ? ($inhouseRattingPositive * 100) / $inhouseReviewDataCount
            : 0;
        ...
        $pageSize    = $request->get('limit', DEFAULT_DATA_LIMIT);
        $sellers     = $sellers->forPage($currentPage, $pageSize);
    ...
    public function more_sellers()
    {
        $topVendorsList = Shop::active()
            ...
            ->with([
                ...
                'seller.shop' => function ($query) {
                    $query->with('governorates');
                }
            ])
            ->get()
