...
class ForgotPasswordController extends Controller
{
    ...
    {
        // 1) Validate the incoming identity (email or phone)
        $request->validate([
            'identity' => 'required|string',
        ]);

        $identity = $request->input('identity');
        // 2) Detect email vs phone
        $by = filter_var($identity, FILTER_VALIDATE_EMAIL) ? 'email' : 'phone';

        // 3) Lookup customer
        $customer = $this->customerRepo->getByIdentity(filters: [$by => $identity]);
        if (! $customer) {
            Toastr::error(translate('No_such_user_found'));
            return back();
        }

        // 4) Store identity in session
        session()->put('forgot_password_identity', $identity);

        // —— EMAIL FLOW —————————————————————————————————————
        if ($by === 'email') {
            // a) Generate token & build URL
            $token = Str::random(120);
            $url   = route('customer.auth.reset-password', [
            ...
            // c) Fetch email‐template settings
            $template    = getWebConfig(name: 'forgot_password_email_template') ?? [];
            $socialMedia = getWebConfig(name: 'social_media')                  ?? [];

            // —–––––– Provide the two keys index.blade.php expects –––––––—
            $template['user_type']            = 'customer';
            // match this to the filename “forgot-password.blade.php” in customer-mail-template/
            $template['template_design_name'] = 'forgot-password';
            // —––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––—

            // d) Send the email using your PasswordResetMail Mailable
            Mail::to($identity)
            ...
            Toastr::success(
                ...
                translate('Password_reset_url_sent')
            );
            return back();
        }

        // —— PHONE / OTP FLOW ——————————————————————————————————
        // a) Throttle repeated sends
        $interval = getWebConfig(name: 'otp_resend_time') ?? 60;
        $existing = $this->phoneOrEmailVerificationRepo->getFirstWhere([
            'phone_or_email' => $identity
        ]);
        if ($existing && Carbon::parse($existing->created_at)->diffInSeconds() < $interval) {
            $wait = $interval - Carbon::parse($existing->created_at)->diffInSeconds();
            Toastr::error(
                translate('please_try_again_after_')
                . CarbonInterval::seconds($wait)->cascade()->forHumans()
            );
            return back();
        }

        // b) Generate & persist a 6-digit OTP
        ...
        // c) TODO: Send $otp via your SMSModule
        // SMSModule::send($identity, "Your OTP is {$otp}");

        Toastr::success(translate('OTP_sent_successfully'));
        return redirect()
            ->route('customer.auth.otp-verification', [
                'identity' => base64_encode($identity),
            ]);
    }
