...
class VendorController extends BaseController
{
    ...
    public function getAddView(Request $request): View
    {
        ...
        return view(Vendor::ADD[VIEW],compact('governorates'));
    }
    ...
    public function add(VendorAddRequest $request): JsonResponse
    {
        ...
        // Add the shop related data for the vendor registration
        $shopData = $this->shopService->getAddShopDataForRegistration(request: $request, vendorId: $vendor['id']);
        $shop = $this->shopRepo->add($shopData);

        // Add initial wallet data for the vendor
        $this->vendorWalletRepo->add($this->vendorService->getInitialWalletData(vendorId: $vendor['id']));

        // Associate the vendor with the selected governorates (if provided)
        if ($request->has('governorate_ids')) {
            $vendor->governorates()->sync($request->input('governorate_ids'));
        }

        // Associate the shop with the selected governorates (if this relationship exists as well)
        if ($request->has('governorate_ids')) {
            $shop->governorates()->sync($request->input('governorate_ids'));
        }

        // Prepare data for the event of vendor registration
        $data = [
        ...
        // Trigger the event for vendor registration
    ...
    public function getView(Request $request, $id, $tab = null): View|RedirectResponse
    {
        ...
        $seller = $this->vendorRepo->getFirstWhere(
            ...
            relations: ['orders', 'product','governorate']
        );
        ...
        if ($tab == 'order') {
         ...
         // Calculate total sales for the seller, excluding returned orders
        // Retrieve points conversion rate from settings
        $conversionSetting = BusinessSetting::where('type', 'seller_points_setting')->first();
        $conversionRate = (int)($conversionSetting->value ?? 100);

    // Use total earning from wallet
        $totalSales = $seller->wallet?->total_earning ?? 0;

    // Calculate seller points
        $sellerPoints = \App\Models\Seller::where('id', $id)
            ->value('points') ?? 0;
    ...
    /**
     * Update the seller's accumulated points.
     *
    ...
    public function updatePoints(Request $request, int $id)
    {
        ...
        // Find vendor and update points
        $vendor = \App\Models\Seller::findOrFail($id);
        $vendor->points = $data['points'];
        $vendor->save();

        return back()->with('success', translate('Seller points updated successfully.'));
    }
