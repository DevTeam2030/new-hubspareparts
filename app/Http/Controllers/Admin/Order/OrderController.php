...
class OrderController extends BaseController
{
    ...
    public function getListView(Request $request, string $status)
    {
        ...
        // mark all unchecked orders as checked = 1
        ...
        // seller filter logic...
        $vendorId = $request->input('seller_id') === '0' ? 1 : $request->input('seller_id');
        if ($request->has('seller_id')) {
            if ($request->input('seller_id') === 'all') {
                $vendorIs = 'all';
            } elseif ($request->input('seller_id') === '0') {
                $vendorIs = 'admin';
            } else {
                $vendorIs = 'seller';
            }
        ...
        if ($request->filled('customer_id') && $request->input('customer_id') !== 'all') {
            $customer = $this->customerRepo->getFirstWhere(
                params: ['id' => $request->input('customer_id')]
            );
        }

        $vendorId    = $request->input('seller_id');
    ...
    public function getView(string|int $id, DeliveryCountryCodeService $service, OrderService $orderService): View|RedirectResponse
    {
        ...
        // Retrieve the order with relationships
        ...
        if ($order) {
            // Check if any item in the order is physical
            ...
            // Find any linked orders that share the same order_group_id
            $whereNotIn = [
            ...
            // Count total delivered for the same seller
            ...
            // Check if order is only digital
            ...
            if ($order['order_type'] == 'default_type') {
                ...
                return view(Order::VIEW_POS[VIEW], compact('order', 'companyName', 'companyWebLogo', 'orderCount','governorates'));
            }
        } else {
    ...
    public function generateInvoice($groupId)
    {
    ...
    ): JsonResponse
    {
        ...
        if (
            ...
            $deductibleFees = ($order['shipping_cost'] ?? 0) + ($order['service_fee'] ?? 0);


            $amountForPoints = usdToDefaultCurrency(
                amount: $order['order_amount'] - $deductibleFees
            );

          // add loyalty point for customer
            $this->loyaltyPointTransactionRepo->addLoyaltyPointTransaction(
                userId:          $order['customer_id'],
                reference:       $order['id'],
                amount:          $amountForPoints,
        ...
        if ($request['order_status'] == 'delivered' && $order['seller_id'] != null) {
            ...
            //
            // : calculate & add seller loyalty points ——↓
            //
            $pointsRate = (int) getWebConfig(name: 'seller_points_setting');
            if ($pointsRate > 0) {
                // earned points = floor(order_amount ÷ rate)
                $earned = (int) floor($order['order_amount'] / $pointsRate);
                if ($earned > 0) {
                    // increment the seller’s points column
                    \App\Models\Seller::where('id', $order['seller_id'])
