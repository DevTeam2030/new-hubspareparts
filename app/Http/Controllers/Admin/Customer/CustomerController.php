...
class CustomerController extends BaseController
{
    ...
    {
        // 1) Retrieve the customer along with their saved addresses
        $customer = $this->customerRepo->getFirstWhere(
            params:    ['id' => $id],
            relations: ['addresses']
        );

        // 2) If customer not found, show error and redirect back
        if (!isset($customer)) {
            Toastr::error(translate('customer_Not_Found'));
            return back();
        }

        // 3) Fetch all orders for this customer (no pagination) to compute status counts
        $allOrders = $this->orderRepo->getListWhere(
            orderBy:     ['id' => 'desc'],
            searchValue: $request['searchValue'],
            filters:     ['customer_id' => $id, 'is_guest' => '0'],
            dataLimit:   'all'
        );

        // 4) Initialize counters for each status category
        $orderStatusArray = [
            'total_order' => 0,
            'ongoing'     => 0,
            'completed'   => 0,
        ...
        // 5) Tally up each order into its status bucket
        $allOrders->each(function ($order) use (&$orderStatusArray) {
        ...
        // 6) Fetch the same orders again, this time paginated for display
        ...
        // 7) Group the paginated orders by their order_group_id
        $groupedOrders = $orders->groupBy('order_group_id');

        // 8) Return the view, passing all needed variables
        return view(
            Customer::VIEW[VIEW],
            compact(
                'customer',        // the customer model
                'orders',          // paginated orders for listing
                'groupedOrders',   // same orders grouped by batch
                'orderStatusArray' // counts for each status badge
            )
        );
    }
