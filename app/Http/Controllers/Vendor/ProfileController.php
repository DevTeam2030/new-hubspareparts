...
class ProfileController extends BaseController
{
    ...
    public function getUpdateView(string|int $id):View|RedirectResponse
    {
        ...
        return view(Profile::UPDATE[VIEW],compact('vendor','shopBanner','governorates'));
    }
...
//    }
    ...
    public function update(VendorRequest $request, string|int $id): JsonResponse
    {
        // Get the vendor based on the provided ID
        $vendor = $this->vendorRepo->getFirstWhere(['id' => $id]);

        // Get the updated vendor data from the request
        $vendorData = $this->vendorService->getVendorDataForUpdate(request: $request, vendor: $vendor);

        // Update the vendor information in the database
        $this->vendorRepo->update(id: $id, data: $vendorData);

        // This will update the pivot table to reflect the selected governorates for the vendor
        $vendor->governorates()->sync($request->governorate_ids); // Linking the vendor to the selected governorates
        $vendor->shop->governorates()->sync($request->governorate_ids);
