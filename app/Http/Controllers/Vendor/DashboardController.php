...
class DashboardController extends BaseController
{
    ...
    public function getView():View
    {
        ...
        // Retrieve conversion rate from business settings (e.g., every 100 EGP = 1 point)
        $conversionSetting = BusinessSetting::where('type', 'seller_points_setting')->first();
        $conversionRate = (int)($conversionSetting->value ?? 100);

        // Get seller's total earning from wallet
        $totalEarning = $vendorWallet->total_earning ?? 0;

       // Calculate seller points based on total earning and conversion rate
        $sellerPoints = \App\Models\Seller::where('id', $vendorId)
            ->value('points') ?? 0;
        $dashboardData = [
