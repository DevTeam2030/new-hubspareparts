    ...
    public function boot(): void
    {
        ...
        if (!App::runningInConsole()) {
            ...
            try {
                ...
                // ─── SMTP OVERRIDE FROM business_settings.mail_config ───────────
                $mail = getWebConfig(name: 'mail_config');
                if ($mail && !empty($mail['status']) && $mail['status'] == '1') {
                    Config::set('mail.default',                    'smtp');
                    Config::set('mail.mailers.smtp.host',          $mail['host']);
                    Config::set('mail.mailers.smtp.port',          $mail['port']);
                    Config::set('mail.mailers.smtp.encryption',    strtolower($mail['encryption']));
                    Config::set('mail.mailers.smtp.username',      $mail['username']);
                    Config::set('mail.mailers.smtp.password',      $mail['password']);
                    Config::set('mail.from.address',               $mail['email_id']);
