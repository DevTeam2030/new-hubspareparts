...
class OrderManager
{
    ...
    public static function generate_order($data)
    {
        ...
        if ($free_delivery['status'] && $free_delivery['shipping_cost_saved'] > 0 && $couponProcess['coupon_type'] != 'free_delivery') {
            $is_shipping_free = 1;
            $freeShippingDiscount = CartManager::get_shipping_cost(groupId: $cart_group_id, type: 'checked');
...
//        }
        ...
        // Get shipping cost from session or request
        $finalShippingCost = 0;

        if ($req && isset($req['shipping_fee'])) {
            $finalShippingCost = (float) $req['shipping_fee'];
        } elseif ($req && isset($req['shipping_cost'])) {
            $finalShippingCost = (float) $req['shipping_cost'];
        } else {
            $finalShippingCost = (float) session('shipping_cost', 0);
        }


        $selectedTime  = null;

// Read the saved delivery date from session

        $serviceFee = $req && isset($req['service_fee'])
            ? (float) $req['service_fee']
            : (float) session('service_fee', 0);

        $deliveryDate = $req['delivery_date']
            ?? session('selected_delivery_date')
            ?? null;

        if (isset($req['selected_delivery_time']) && !empty($req['selected_delivery_time'])) {
            $selectedTime = $req['selected_delivery_time'];
        }
        elseif (session()->has('selected_delivery_time')) {
            $deliveryTimeId = session('selected_delivery_time');

            //  Fetch the delivery‐time record
            $deliveryTime = DeliveryTime::find($deliveryTimeId);

            if ($deliveryTime) {
                // 5. Format start/end with AM/PM and combine them
                $start = DateHelper::formatTimeWithPeriod($deliveryTime->start_time);
                $end   = DateHelper::formatTimeWithPeriod($deliveryTime->end_time);
                $selectedTime = "{$start} - {$end}";

                // 6. Only auto‐calculate the date if it wasn’t stored already
                if (!$deliveryDate) {
                    $now    = Carbon::now();
                    $startC = Carbon::parse($deliveryTime->start_time);
                    $endC   = Carbon::parse($deliveryTime->end_time);

                    if ($now->between($startC, $endC)) {
                        // If now is within the window, schedule for tomorrow
                        $deliveryDate = $now->addDay()->format('Y-m-d');
                    }
                    elseif ($now->greaterThanOrEqualTo($endC)) {
                        // If now is past the window, also schedule for tomorrow
                        $deliveryDate = $now->addDay()->format('Y-m-d');
                    }
                    else {
                        // If now is before the window, schedule for today
        ...
        $or = [
            ...
            'selected_delivery_time' => $selectedTime,
            'delivery_date'          => $deliveryDate,
            'service_fee' => $serviceFee,
        ];


        // Offline payment insertion
        ...
        foreach (CartManager::getCartListQuery(groupId: $data['cart_group_id'], type: 'checked') as $cartSingleItem) {
            $product = Product::where(['id' => $cartSingleItem['product_id']])
                ->with([
                    'digitalVariation',
            ...
            $digitalProductVariation = DigitalProductVariation::with(['storage'])
                ->where([
                    'product_id' => $cartSingleItem['product_id'],
                    'variant_key' => $cartSingleItem['variant']
                ])->first();
            ...
            $orderDetails = [
                ...
                'collection_id' => $cartSingleItem['collection_id'],
                'created_at' => now(),
                'updated_at' => now()
            ];
            ...
            $checkWishlist = Wishlist::where('product_id' , $cartSingleItem['product_id'])
                ->where('wishlist_collection_id' , $cartSingleItem['collection_id'] )->first();
            if($checkWishlist){
                if($checkWishlist->quantity <= $cartSingleItem['quantity']){
                    $checkWishlist->delete();
                }else{
                    $checkWishlist->quantity =  $checkWishlist->quantity -  $cartSingleItem['quantity'];
                    $checkWishlist->save();
                }
               $collection =  WishlistCollection::find($cartSingleItem['collection_id']);
                if ($collection->wishlist_items()->count()  == 0 ){
                    $collection->delete();
                }
            }
        }
        ...
        try {
            ...
            // سجل رسالة الخطأ مع تفاصيل الطلب
            \Illuminate\Support\Facades\Log::error("Order #{$order_id} — Mail dispatch error: ".$e->getMessage(), [
                'order_id'  => $order_id,
                'exception' => $e,
            ]);
        }
    ...
    public static function storeInvoice(int $orderId): string
    {
        // 1) Retrieve the main order by its ID
        $mainOrder = \App\Models\Order::findOrFail($orderId);

        // 2) Load all orders in the same group, including related details
        $orders = \App\Models\Order::with([
            ...
            ->where('order_group_id', $mainOrder->order_group_id)
            ->get();

        // 3) Load the customer data for the main order
        $order = \App\Models\Order::with('customer')
            ->where('order_group_id', $mainOrder->order_group_id)
            ->first();

        // 4) Calculate totals: shipping, service fee, and overall order amount
        $totalShippingCost = $order->shipping_cost;
        $totalServiceFee   = $order->service_fee ?? 0;
        $totalOrdersAmount = $orders->sum('order_amount');
        $grandTotal        = $totalOrdersAmount + $totalShippingCost + $totalServiceFee;

        // 5) Fetch invoice settings safely
        $rawSettings = getWebConfig(name: 'invoice_settings');
        if (is_string($rawSettings)) {
            // decode JSON string
            $invoiceSettings = json_decode($rawSettings, true) ?: [];
        } elseif (is_array($rawSettings)) {
            // already an array
            $invoiceSettings = $rawSettings;
        } else {
            // fallback
            $invoiceSettings = [];
        }

        // 6) Prepare logo data or URL
        $logoData = '';                    // optional base64-encoded logo
        $logoUrl  = asset('images/logo.png');

        // 7) Render the invoice view with all required data
        $mpdf_view = \View::make('web-views.order.invoice', [
            'orders'            => $orders,
            'order'             => $order,
        ...
        // 8) Generate and store the PDF, returning its path
        return self::storePdf(
            view:        $mpdf_view,
            filePrefix:  'order_invoice_',
            filePostfix: $order->id,
            pdfType:     'invoice',
            requestFrom: 'web'
        );
    }
    ...
    public static function getOrderTotalPriceSummary($order): array
    {
        ...
        // sum up each order line
        foreach ($order->details as $detail) {
            $linePrice = $detail['price'] * $detail['qty'];

            $itemPrice         += $linePrice;
            $subTotal          += $linePrice;
            $totalProductPrice += $linePrice;
            $itemDiscount      += $detail['discount'];
            $taxTotal          += $detail['tax'];
            $totalItemQuantity += $detail['qty'];
        }

        // base total: price + tax - item discounts
        $total = $itemPrice + $taxTotal - $itemDiscount;

        // shipping fee from order
        $shipping = $order['shipping_cost'];

        // calculate extra discount (percent or fixed)
        if ($order['extra_discount_type'] == 'percent') {
            $extraDiscount = ($totalProductPrice * $order['extra_discount']) / 100;
        } else {
        ...
        // if shipping is free, subtract full shipping cost
//        if ($order['is_shipping_free'] == 1) {
//            $deliveryFeeDiscount = $shipping;
//        }

        // service fee stored on order (or zero)
        $serviceFee = $order['service_fee'] ?? 0;

        // final grand total: add shipping & service fee, subtract discounts
        $grandTotal = $total
            + $shipping
            + $serviceFee
            - $extraDiscount
        ...
        return [
            ...
            'serviceFee'          => $serviceFee,    // added service fee
            'totalItemQuantity'   => $totalItemQuantity,
            'totalAmount'         => $grandTotal,    // use grand total
            'paidAmount'          => $order['paid_amount'],
            'changeAmount'        => $order['paid_amount'] - $grandTotal,
        ];
    }
