...
//        $data['color_image'] = $color_image;
    ...
    public static function product_data_formatting($items, $multi_data = false)
    {
        if (! $items) {
            return null;
        }
        ...
        if ($multi_data) {
            foreach ($items as $item) {
                ...
                // ensure seller.governorates is loaded
                $item->loadMissing('seller.governorates');

                // Base product data (colors, images, variations, etc.)
                $itemData = Helpers::set_data_format($item);

                // Seller governorates if relation is loaded and seller exists
                if ($item->relationLoaded('seller') && $item->seller) {
                    $itemData['governorates'] = $item->seller
                        ->governorates
                        ->map(fn($g) => ['id' => $g->id, 'name' => $g->name])
                        ->toArray();
                } else {
                    // No seller or not loaded → empty array
                    $itemData['governorates'] = [];
                }
        ...
        // Single item → just return base product data
        return Helpers::set_data_format($items);
    }
