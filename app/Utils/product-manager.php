...
class ProductManager
{
    ...
    public static function getNewArrivalProducts($request, $limit = 10, $offset = 1)
    {
        ...
        $products = Product::active()
            ->with(['rating', 'tags', 'seller.shop', 'flashDealProducts.flashDeal',  'seller.governorates',
                'seller.shop.governorates', 'clearanceSale' => function ($query) {
                return $query->active();
            }])
            ->withCount(['reviews', 'wishList' => function ($query) use ($user) {
                $query->where('customer_id', $user != 'offline' ? $user->id : '0');
    ...
    {
        $colorsMerge = [];
        $colorsCollection = Product::active()
            ->where('colors', '!=', '[]')
    ...
    {
        // Include counts for order details, reviews, and wishlist and calculate the average rating from reviews
        $query = $query->withCount(['orderDetails', 'reviews', 'wishList'])->withAvg('reviews', 'rating');

        // Add governorate filtering if a user (customer or seller) is logged in and has a governorate_id
...
//        });
        ...
        if ($featuredProductSortBy && ($featuredProductSortBy['custom_sorting_status'] == 1)) {
            // Sort products based on temporary close status if required
            ...
            // Hide out-of-stock products if required
            ...
            // Apply different sorting criteria based on the configuration
        ...
        // Fallback: simple ordering of featured products by descending ID
    ...
    {
        // Retrieve configuration for new arrival product list priority
        $newArrivalProductSortBy = getWebConfig(name: 'new_arrival_product_list_priority');

        // Add count and average rating calculations to the query
        $query = $query->withCount(['orderDetails', 'reviews', 'wishList'])->withAvg('reviews', 'rating');

        if ($newArrivalProductSortBy && ($newArrivalProductSortBy['custom_sorting_status'] == 1)) {

            // Adjust query based on temporary close status
            ...
            // Filter products by creation date based on the specified duration
            ...
            // Apply sorting based on the configuration
            ...
            // Apply additional merging for out-of-stock products if configuration is set to 'desc'
            if ($newArrivalProductSortBy['out_of_stock_product'] == 'desc') {
                $query = self::mergeStockAndOutOfStockProduct(query: $query);
            }

            // Sort the collection by temporary close status if required
            if ($newArrivalProductSortBy['temporary_close_sorting'] == 'desc') {
                $query = $query->sortBy('is_shop_temporary_close');
            }

            // If dataLimit is not "all", apply pagination manually on the collection
            if ($dataLimit != 'all') {
                $currentPage = $offset ?? Paginator::resolveCurrentPage('page');
                $totalSize = $query->count();
                $results = $query->forPage($currentPage, $dataLimit);
        ...
        // If custom sorting is disabled, just sort by order details count in descending order
        $query = $query->orderBy('order_details_count', 'desc');
    ...
    {
        // Build the cache key for the flash deal
        ...
        // Retrieve the flash deal from cache or database
        ...
        // If a flash deal exists, retrieve its associated products
        if ($flashDeal) {
            // Build the base query for flash deal products
            ...
            // Add governorate filtering if the logged-in user has a governorate_id
            $query->when(auth('customer')->check(), function ($query) {
                if (auth('customer')->check()) {
                    $govs = [ auth('customer')->user()->governorate_id ];
                } elseif (auth('seller')->check()) {
            ...
            // Pass the resulting query to the ProductManager method to further apply sorting and prioritization
