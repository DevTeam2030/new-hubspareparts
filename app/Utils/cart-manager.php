...
class CartManager
{
    ...
    public static function addToCartPhysicalProduct($request, $product, $shippingType, $sellerShippingList): array
    {
        ...
        if ($request['buy_now'] == 1) {
            ...
            if ($product['product_type'] == 'physical' && $shippingType == 'order_wise') {
                // Skip shipping-method logic entirely and mark cart as checked
                $cart->update([
                    'is_checked' => 1,
                    // uncomment next line if you have a shipping_cost column on cart
                    // 'shipping_cost' => 0,
                ]);

                // recalculate free-delivery threshold if needed
        ...
        // After main cart creation
        if ($user != 'offline') {
            $relatedAccounts = self::getRelatedAccounts(); // adjust relation according to your DB

            foreach ($relatedAccounts as $relatedUser) {
                // duplicate cart array but replace customer_id
                $relatedCartArray = $cartArray;
                $relatedCartArray['customer_id'] = $relatedUser;
                $relatedCartArray['is_guest'] = 0;

                // Check if already exists
                $relatedCart = Cart::where([
                    'product_id' => $request['id'],
                    'customer_id' => $relatedUser,
                    'is_guest' => 0,
                    'variant' => $string,
                ])->first();

                if ($relatedCart) {
                    $relatedCartArray['cart_group_id'] = $relatedCart['cart_group_id'];
                    Cart::where(['id' => $relatedCart['id']])->update($relatedCartArray);
                } else {
                    $relatedCartArray['cart_group_id'] =
                        $relatedUser . '-' . Str::random(5) . '-' . time();
                    Cart::insert($relatedCartArray);
                }
            }
        }
    ...
    public static function addToCartDigitalProduct($request, $product, $shippingType, $sellerShippingList): array
    {
        ...
        // After main cart creation
        if ($user != 'offline') {
            $relatedAccounts = self::getRelatedAccounts(); // adjust to your relation
    ...
    public static function  getRelatedAccounts()
    {
