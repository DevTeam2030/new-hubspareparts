...
Route::group(['namespace' => 'Web', 'middleware' => ['maintenance_mode', 'guestCheck']], function () {
    ...
    Route::controller(UserProfileController::class)->group(function () {
        ...
        Route::get('account-order-details/{id}', 'account_order_details')->name('account-order-details')->middleware('customer');
        ...
        Route::get('account-roles', 'account_roles')->name('account-roles')->middleware('customer');
        Route::post('account-roles-update/{id}', 'updateRoles')->name('account-roles-update')->middleware('customer');
        Route::get('account-users', 'allUsers')->name('account-users')->middleware('customer');
        Route::get('account-wishlist-collections', 'wishlistCollections')->name('account-wishlist-collections')->middleware('customer');
        Route::get('view-wishlist-collection/{id}', 'viewWishlistCollection')->name('view-wishlist-collection')->middleware('customer');
        Route::get('create-wishlist-collection', 'createWishlistCollection')->name('create-wishlist-collection')->middleware('customer');
        Route::post('store-wishlist-collection', 'storeWishlistCollection')->name('store-wishlist-collection')->middleware('customer');
        Route::post('approve-eng', 'approveEng')->name('approve-eng')->middleware('customer');
        Route::post('approve-proc', 'approveProc')->name('approve-proc')->middleware('customer');
        Route::post('store-user', 'storeUser')->name('store-user')->middleware('customer');
        Route::get('edit-user/{id}', 'editUser')->name('edit-user')->middleware('customer');
        Route::post('update-user', 'updateUser')->name('update-user')->middleware('customer');
        Route::get('user-delete/{id}', 'deleteUser')->name('user-delete')->middleware('customer');
        Route::post('wishlist-collections-delete/{id}', 'deleteWishlistCollection')->name('delete-wishlist-collection')->middleware('customer');
        Route::get('wishlist-collections-edit/{id}', 'editWishlistCollection')->name('edit-wishlist-collection')->middleware('customer');
        Route::post('wishlist-collections-update', 'updateWishlistCollection')->name('update-wishlist-collection')->middleware('customer');
...
if (!$isGatewayPublished) {
    ...
    Route::post('store-shipping-details-in-session', function (Illuminate\Http\Request $request) {
        if($request->has('shipping_cost')){
            session(['shipping_cost' => $request->shipping_cost]);
        }
        if($request->has('service_fee')){
            session(['service_fee' => $request->service_fee]);
        }
        if($request->has('selected_delivery_time')){
            session(['selected_delivery_time' => $request->selected_delivery_time]);
        }
        if($request->has('selected_delivery_date')){
            session(['selected_delivery_date' => $request->selected_delivery_date]);
        }
        return response()->json(['success' => true]);
    })->name('store-shipping-details-in-session');
    Route::get('/invoice/{groupId}/download', [UserProfileController::class, 'downloadInvoicePdf'])
